...
import useWindowSize from "../../CustomHooks/useWinowSize";
...
const UserCard = ({feed , isProfile}) => {
    ...
    const isMediumScreen = windowWidth >= 768 && windowWidth < 1280;
    const cardScale = showDetails ? (isMediumScreen ? "scale-[0.95]" : "scale-100") : (isMediumScreen ? "scale-[1.1]" : "scale-100");

    // Calculate widths based on whether detail is open
    const mainCardWidth = showDetails
      ? "md:w-[20rem]" // Shrink when details visible
      : "md:w-[32rem]"; // Expand when details hidden

    const detailSectionWidth = showDetails
      ? "md:w-[20rem]"
      : "md:w-0";
  ...
  return (
    <div className={`h-[40rem] md:h-[40rem] max-w-screen flex flex-col md:flex-row justify-center items-center ${isProfile ? "mr-[0.4rem]" : "mr-0"}`}> {/* << FIXED HEIGHT HERE */}
    ...
    {/* ${isVisible ? `${showDetails ? `${ isProfile ? "scale-75" : "scale-80"}` : `${ isProfile ? "scale-77" : "scale-90"}`}  md:scale-100 opacity-100` : "scale-0 opacity-0"} */}
      <div
        className={`relative transform transition-all duration-500 ease-in-out 
                    ...
                    ${isVisible ? `${cardScale} opacity-100` : "scale-0 opacity-0"} `}
      ...
      >
        ...
        <div
          ...
          // `}
          className={`h-full flex-[1] md:flex-[1] transition-all duration-700 ease-in-out max-w-screen ${mainCardWidth}`}
        >
          <Card
            feed={feed}
        ...
        <div
          ...
          className={`
            ...
            ${showDetails ? "opacity-100 flex-[2]" : "opacity-0 flex-[0]"} 
            ${detailSectionWidth}
          `}
        >
          {renderDetail && (
...
//   return (
//     <div className={`h-[40rem] md:h-[40rem] max-w-screen flex flex-col md:flex-row ${isProfile ? "mr-[0.4rem]" : "mr-0"} `} > {/* << FIXED HEIGHT HERE */}
//     {/* <div className="flex w-full h-full md:flex-row flex-col"> */}
//       <div
//         className={`relative transform transition-all duration-500 ease-in-out 
//                     p-1 border border-gray-600 bg-gray-600 shadow-black shadow-xl 
